...
- **Message history** — Use `mcp__nanoclaw__search_messages` to search past messages in this chat. Supports text search (`query`), date filtering (`before`/`after`), and pagination (`limit`). Works for all messages including those sent by scheduled tasks. Do NOT use `feishu history`, `grep conversations/`, or `sqlite3` for message history — they are unreliable and incomplete. Always use `search_messages`.
...
- **PDF files (.pdf)** — To read: use the Read tool (built-in multimodal support, use `pages` param for large PDFs). To create from HTML: use **WeasyPrint** (`python3 -c "from weasyprint import HTML; HTML('file.html').write_pdf('out.pdf')"`). To convert PPTX/DOCX to PDF: use **LibreOffice** (`libreoffice --headless --convert-to pdf file.pptx --outdir /tmp/`). Do NOT use Chromium/Playwright for PDF generation — crashpad crashes in this container.
- **PPT files (.pptx)** — Use `python-pptx`: write a Python script with `from pptx import Presentation`. Both `python-pptx` and `pptxgenjs` (npm) are available. To convert to PDF after generating: `libreoffice --headless --convert-to pdf file.pptx --outdir /tmp/`.
...
- **Python** — Use `python3` (not `python`). For pip installs: `pip3 install --break-system-packages <pkg>` (system Python, no venv).
